...
from lm.model import DecoderLM, MultiHeadAttention
...
def test_self_attention():
    ...
    attn_ref = rearrange(attn_ref_multihead, "b h s hd -> b s (h hd)")
    print_tensor(attn_ref, "attn_ref (pytorch)")
    print_tensor(attn, "attn (scratch)")
...
def print_tensor(t: torch.Tensor, name: str):
    """Helper function to print a tensor's name, shape, dtype, and values."""
    print(f"--- {name} ---")
