...
def random_batch_sampler(
    ...
    Returns:
        ...
        tokens. Each batch has shape (B, S). Every sequence in the batch is
    ...
    while True:
        ...
        # The high argument for randint is exclusive, so we add 1 to make
        # the last possible sequence start index inclusive.
...
def sequential_batch_sampler(
    ...
    Returns:
        ...
        shape (B, S). Every sequence in the batch is a contiguous subsequence
    ...
    def get_lr(t: int) -> float:
        ...
        if t <= num_warmup_steps:
            if num_warmup_steps > 0:
                ...
                lr = max_lr  # No warmup, start at max
...
def compute_language_modeling_loss(
    ...
    # The model predicts the next token, so we compare its output
    # at each position with the actual next token in the sequence.
    # The logits for the last token are ignored as there is no label.
    # The first token in the input is not predicted, so we start labels from the second token.
    labels = input_ids[:, 1:].contiguous()
    logits = logits[:, :-1, :].contiguous()

    # Reshape for cross_entropy, which expects (N, C) and (N)
    # N = Batch Size * Sequence Length, C = Vocabulary Size
...
def main():
    ...
    wandb.init(project="llms-hw2-best", config=OmegaConf.to_container(config))
    ...
    if model_disk_size_MB > 98:
        print(
            ...
            "The largest model size allowed is 100MB, "
